...
If your android tablet does not have Stylus support, you can choose Non S-Pen mode.
...
* image:icons/gp_copy.png[Copy Document] Copy selected document
* image:icons/gp_pin.png[Pin Document]  Pin selected document
* image:icons/gp_delete.png[Delete Document] Delete selected document
...
Step 4) (Optional) In order to convert a temporary group to a persistent group, long press this temporary group with finger

image::screenshots/upgrade-tmp-group-to-persistent-one.png[Grouping Step 4]

[NOTE]
You can also convert with *group button* image:icons/group.png[Group] on the left bottom toolbar.
...
Step 2) Tap the *Ungroup* image:icons/ungroup.png[Ungroup] to ungroup

image::screenshots/ungrouping-step-3.png[Ungrouping Step 1]


[NOTE] 
The *Ungroup* image:icons/ungroup.png[Ungroup] is located on the left bottom toolbar. +
You can also use gesture to ungroup.


==== Ungrouping Group with gesture
...
[NOTE]
Two gray dot *Link Handles* are located on group border left and right side.


Step 4) Drop it on a destination group
